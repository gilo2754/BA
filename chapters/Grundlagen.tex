...
Von dieser Stelle der Arbeit an liegt der Fokus auf den Cloud-Diensten von Amazon Web Services, die als \textit{AWS-Dienste} bezeichnet werden. Einer der am häufigsten genutzten AWS-Dienste ist \textit{Amazon Elastic Computing Instances EC2}, mit dem virtuelle Maschinen erstellt werden können\footnote{Cloud infrastructure services vendor market share worldwide from 4th quarter 2017 to 3rd quarter 2021.\cite{STA4}}. Ein weiterer wichtiger Dienst ist \textit{Amazon Simple Storage Service (S3)}, der zum Speichern von Objekten\footnote{Objekte sind in AWS die Grundeinheit in welchen Dateien in den Amazon S3-Speichereinheiten gespeichert werden. Neben den Objekten werden Metadaten, wie das Datum der Objekterstellung und das Datum der letzten Aktualisierung gespeichert.} verwendet wird. Deshalb konzentrieren sich in dieser Arbeit die Überwachungs- und Optimierungsmaßnahmen hauptsächlich auf EC2-Instanzen und Amazon S3. Amazon Elastic Computing EC2-Instanzen werden im Folgenden in dieser Arbeit als \textit{EC2-Instanzen} und Amazon Simple Storage Service als \textit{Amazon S3} oder \textit{Amazon S3-Speichereinheiten} bezeichnet. %\subsubsection*{Amazon Elastic Computing Instances EC2}
...
\\\\
Wie Lynn Langit, eine erfahrene Cloud-Architektin, feststellt, können bis zu 80\% der AWS-Rechnung aus Gebühren für EC2-Instanzen bestehen\footnote{LinkedIn Learning: AWS Controlling Cost by Lynn Langit. Minute 0:20-0:45\cite{LINK2}}. Laut des AWS Solutions Architekten Daniel Peña Silva\footnote{LinkedIn: Listado de todos los Servicios de AWS.\cite{LINK1}} ist Amazon S3 einer der am häufigsten genutzten AWS-Dienste.
